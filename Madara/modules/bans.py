...
from Madara.modules.helper_funcs.extraction import extract_user_and_text
from Madara.modules.helper_funcs.string_handling import extract_time
from Madara.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text("⚠️ ᴄᴀɴ ɴᴏᴛ ʙᴀɴ ᴀᴅᴍɪɴ.")
        return log_message
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if res := chat.unban_member(user_id):
        ...
        bot.sendMessage(
            chat.id,
            f"{mention_html(member.user.id, html.escape(member.user.first_name))} [<code>{member.user.id}</code>] ᴋɪᴄᴋᴇᴅ ʙʏ {mention_html(user.id, html.escape(user.first_name))}",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
